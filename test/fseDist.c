...
int FSEU16d_noCompression(void* dest, const U16* source, int sourceSize)
{
...
int FSED_compressU16 (void* dest, const U16* source, int sourceSize, int memLog)
{
    ...
    // early out
    if (sourceSize <= 1) return FSEU16d_noCompression (ostart, istart, sourceSize);
    ...
    if ( (op-ostart) >= (sourceSize*2-1) )
        return FSEU16d_noCompression (ostart, istart, sourceSize);
