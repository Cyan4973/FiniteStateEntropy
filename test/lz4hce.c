                       ...
                       extractionType eType)
{
    ...
    if (eType == et_literals)
    {
        const BYTE* src = *anchor;
        token = (*op);
        while (src<*ip) *token++ = *src++;
        *op = token;
        // Prepare next loop
        *ip += matchLength;
        *anchor = *ip;
...
_Search2:
        ...
        {
            if (LZ4HC_encodeSequence(&ip, &op, &anchor, ml, ref, limit, oend, eType)) return 0;
            continue;
        }
...
_Search3:
        ...
        if (ml3 == ml2) // No better match : 2 sequences to encode
        {
            ...
            if (LZ4HC_encodeSequence(&ip, &op, &anchor, ml, ref, limit, oend, eType)) return 0;
            ip = start2;
            if (LZ4HC_encodeSequence(&ip, &op, &anchor, ml2, ref2, limit, oend, eType)) return 0;
            continue;
        }
...
int LZ4_extractHC (const char* source, char* dest, int inputSize, extractionType eType)
{
    ...
    result = LZ4HC_compress_generic (ctx, source, dest, inputSize, 0, noLimit, eType);
